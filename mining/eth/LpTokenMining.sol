...
contract LpTokenMining is StakingInterface {
    using SafeMath for uint256;

    bool internal initTotalRewards = false;
    address public lpToken;

    constructor (uint256 _startTime, uint256 _finishTime, address _lpToken) public {
    ...
    function staking(uint256 _amount) external {
        ...
        require(EIP20Interface(lpToken).allowance(msg.sender, address(this)) >= _amount, "need approve");
        stakingInternal(msg.sender, _amount, lpToken);
    }

    function redeem(uint256 _amount) external {
        redeemInternal(msg.sender, _amount, lpToken);
    }
    ...
    function _balanceOf(address _account) public view returns (uint256) {
